        ...
        if "values" in cell:
            for col, stats in cell["values"].items():
                ...
                if existing is None:
                    ...
                    # Recompute a weighted mean; min/max from both sides.
                    total = prev_n + n_add
                    if total > 0 and "mean" in stats and "mean" in existing:
                        existing["mean"] = round(
                            (existing["mean"] * prev_n + stats["mean"] * n_add) / total, 6
                        )
                    if "min" in stats:
...
def build_freq_result(
    ...
    order: List[Tuple[str, ...]] = []
    for rec in records:
        key = _group_key(rec, by_cols)
        if key not in groups:
            groups[key] = 0
            order.append(key)
        groups[key] += 1
    cells = [
        {"by": _masked_by_map(key, by_cols, pii_kinds), "n": groups[key]}
        for key in order
    ]
