...
|---|---|---|
| Telemetry consent (`SKILL.md`) | P4 | 동의 선택 전에 연구 내용(아이디어·데이터·원고·대화)은 전혀 전송되지 않고, 진행 단계·진입점·이벤트명·플러그인 버전과 익명 토큰·프로젝트 해시를 집계한 퍼널 카운터를 개선에 쓴다는 가치 1줄을 먼저 설명한다 |
...
단, rehearsal 활동은 real 단계 폴더에 어떤 파일도 쓰지 않고 요약을 `rehearsal/` 아래에만
저장하거나 생략합니다. `imported` 상태로 반입된 단계는 완료 무브가 없으므로 `SUMMARY.md`를 만들지 않습니다.
