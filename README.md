...
> `/rf`와 `/researchfellow`는 **같은 명령**입니다 — 편한 쪽을 쓰세요.
> `/rf`가 다른 플러그인과 겹쳐 인식되지 않으면 `/researchfellow`(또는 전체 이름 `/researchfellow:rf`)를 사용하세요.
...
출발해 **실제 문헌(PubMed 등 6개 데이터베이스)**, 근거표·변수 정의, 프로토콜·통계분석계획, 데이터 QC와
...
| | |
|---|---|
| 🔒 **개인정보 보호** | 환자 데이터는 로컬에서만 처리. AI는 원본 데이터 파일을 직접 읽지 않고 집계 결과만 보며, 한 명으로 좁혀지는 결과는 아예 출력되지 않습니다. 주민번호·연락처가 섞여 있으면 자동으로 경고 |
...
| 🚫 **연구 부정 방지 내장** | 합성 결과의 원고 삽입 금지에 더해, 원고 속 숫자와 인용을 근거표와 자동 대조해 근거 없는 수치·가짜 인용을 제출 전에 걸러냅니다 |
...
**Q. 인터넷이 필요한가요?**
문헌 검색에만 필요합니다. PubMed를 비롯해 PMC·Europe PMC·medRxiv·OpenAlex·Crossref까지
6개 학술 데이터베이스를 함께 찾고, 그 외 전 과정은 오프라인으로 완주 가능합니다.
...
- [`skills/researchfellow/references/state-machine.md`](skills/researchfellow/references/state-machine.md) — 상태머신 v3 (artifact DAG, gate 체계)
- [`skills/researchfellow/references/guardrails.md`](skills/researchfellow/references/guardrails.md) — 무결성 가드레일
- `.mcp.json.example` — (선택) 원격 MCP 연동. 없어도 모든 기능 완주 가능

> **명령 이름이 겹칠 때**: 다른 플러그인이 `/rf`를 선점하면 같은 명령의 다른 이름
> `/researchfellow`(또는 전체 이름 `/researchfellow:rf`)를 사용하세요.
